...
def parse_args():
    ...
    parser.add_argument('--regs', nargs='?',default=0.00001, type = float, help='regularization coefficient')

    parser.add_argument('--learning_rate', nargs='?',default=0.00001, type = float,  help='learning rate')
    parser.add_argument('--num_negatives', nargs='?',default=2,  type = int, help='number of negative entries in training')
    parser.add_argument('--mf_dim', nargs='?',default=64,  type = int, help='embedding dimension')
...
if __name__ == '__main__':
    ...
    dataset = Dataset_test(data_path + data +'/', selected_year, num_years_added)
